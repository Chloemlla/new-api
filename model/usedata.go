...
// GetQuotaDatesByChannel 按渠道聚合每个小时的数据看板记录，用于渠道成本分析图表。
func GetQuotaDatesByChannel(startTime int64, endTime int64) (quotaData []*QuotaData, err error) {
	var quotaDatas []*QuotaData
	// 从quota_data表中查询数据，按 channel_id + created_at 聚合
	err = DB.Table("quota_data").
		Select("channel_id, created_at, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used").
		Where("created_at >= ? and created_at <= ?", startTime, endTime).
	...
	channelIDs := make([]int, 0, len(quotaDatas))
	channelIDSet := make(map[int]struct{}, len(quotaDatas))
	for _, row := range quotaDatas {
		if row.ChannelID == 0 {
			continue
		}

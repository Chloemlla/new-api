...
describe('time range expression parsing', () => {
  ...
  test('keeps an always-false && pair as two scalar conditions', () => {
    // `>= 21 && < 6` can never hold. Parsing it as MATCH_RANGE(21, 6) would make
    // the builder emit the overnight `||` form and turn a dormant rule live.
    const groups = tryParseRequestRuleExpr(
      '(hour("Asia/Shanghai") >= 21 && hour("Asia/Shanghai") < 6 ? 2 : 1)'
    )
    expect(groups?.[0].conditions.map((c) => c.mode)).toEqual([
  ...
  test('keeps an always-false && pair scalar when other conditions follow', () => {
    const groups = tryParseRequestRuleExpr(
      '(param("service_tier") == "fast" && hour("Asia/Shanghai") >= 21 && hour("Asia/Shanghai") < 6 ? 2 : 1)'
    )
    expect(groups?.[0].conditions.map((c) => c.mode)).toEqual([
      MATCH_EQ,
      MATCH_GTE,
...
describe('time range round-trip stability', () => {
  ...
  test.each([
    ['always-false && pair', '>= 21 && hour("Asia/Shanghai") < 6'],
    ['overnight || range', '>= 21 || hour("Asia/Shanghai") < 6'],
    ['within-day && range', '>= 9 && hour("Asia/Shanghai") < 12'],
  ])('parse → build preserves the operator of a %s', (_name, tail) => {
    const expr = `(hour("Asia/Shanghai") ${tail} ? 2 : 1)`
    const parsed = tryParseRequestRuleExpr(expr)
    expect(parsed).not.toBeNull()
    expect(buildRequestRuleExpr(parsed ?? [])).toBe(expr)
  })

...
func TestSecurityAccountEncryptedLongPasswordLogin(t *testing.T) {
	...
	require.True(t, ok)
	// Each ciphertext carries its own one-time nonce, so callers that reach
	// DecryptPassword twice must encrypt the payload twice.
	encrypt := func(value string) string {
		nonce, nonceKeyID, err := common.IssuePasswordEncryptionNonce()
		require.NoError(t, err)
		require.Equal(t, keyID, nonceKeyID)
		payload, err := common.Marshal(map[string]string{"nonce": nonce, "password": value})
		require.NoError(t, err)
		key, gcmNonce := make([]byte, 32), make([]byte, 12)
		_, err = rand.Read(key)
		require.NoError(t, err)
		_, err = rand.Read(gcmNonce)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		ciphertext := gcm.Seal(nil, gcmNonce, payload, []byte("password-v2:"+keyID))
		return strings.Join([]string{
			"v2",
			base64.StdEncoding.EncodeToString(wrappedKey),
			base64.StdEncoding.EncodeToString(gcmNonce),
	...
	proof, err := service.VerifySecurityInput(identity, service.VerificationInput{Method: "password", Scope: service.VerificationScopePasswordChange, PasswordEncrypted: encrypt(password), EncryptionKeyID: keyID})
	require.NoError(t, err)
	assert.NotEmpty(t, proof.ProofToken)
	parts := strings.Split(encrypted, ".")
	tampered, err := base64.StdEncoding.DecodeString(parts[3])
	require.NoError(t, err)
	tampered[len(tampered)-1] ^= 1
	parts[3] = base64.StdEncoding.EncodeToString(tampered)

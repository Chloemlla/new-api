...
func GenerateOAuthCode(c *gin.Context) {
	...
	if request.Intent == model.AuthFlowIntentBind || request.Intent == model.AuthFlowIntentVerify {
		...
		// Only the login intent reaches this branch, so the browser binding
		// never weakens the session-authorized bind and verify flows.
		...
		flowPayload.BrowserToken = token
	}
